...
exports.authMiddleware = async (req, res, next) => {
  try {
    ...
    const token = req.headers.authorization?.split(" ")[1]; //this fix...idek what happened here, but it works XD-----------------------
    ...
    //attach the user and email to the current req object
    req.user = user;
    req.email = payload.email; //Extract email from the payload
